...
def simulacia(vstup,druh, name):
    ...
    if counter == len(list_of_table)-1:
        ...
        elif druh=='ntm' or druh =='dtm':
            success_message = ('Turingov stroj cyklí: '+vstup )
        flash(success_message)
    session["counter"] = counter  
    dff = list_of_table[counter]
    dff = dff.set_table_attributes('border="1" class="dataframe table table-hover table-bordered"')
    ...
    if druh == "xtm":
        ...
        if int(crossnumber) == 2:
           return render_template("simulacia_ntm_2.html",form=form,data1 = input_symbols,dataframe = df_tape.to_html(classes=["table-bordered", "table-striped", "table-hover"]), data = dff)
        elif int(crossnumber) == 3:
            return render_template("simulacia_ntm_3.html",form=form,data1 = input_symbols,dataframe = df_tape.to_html(classes=["table-bordered", "table-striped", "table-hover"]), data = dff)
        elif int(crossnumber) == 4:
            return render_template("simulacia_ntm_4.html",form=form,data1 = input_symbols,dataframe = df_tape.to_html(classes=["table-bordered", "table-striped", "table-hover"]), data = dff)
        elif int(crossnumber) == 5:
            return render_template("simulacia_ntm_5.html",form=form,data1 = input_symbols,dataframe = df_tape.to_html(classes=["table-bordered", "table-striped", "table-hover"]), data = dff)
        elif int(crossnumber) == 6:
            return render_template("simulacia_ntm_6.html",form=form,data1 = input_symbols,dataframe = df_tape.to_html(classes=["table-bordered", "table-striped", "table-hover"]), data = dff)
        else:
        ...
        if form.validate_on_submit():
            ...
            if tm:
                try:
                    ...
                    flash(success_message)
        #####ulozenie do PC 
        elif request.form.get('Ulož do PC') == 'Ulož do PC':
            tm = False
            states = parser1.dict_parse(form.states.data)
            states_dict = literal_eval(states)
            input_s = parser1.dict_parse(form.input_symbols.data)
